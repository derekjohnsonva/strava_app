...
async function getActivities(fetch, page = 1, per_page = 60, before = 0, after = 0) {
	...
	let activitySummaries: ActivitySummary[] = await response.json();
	...
	let activities = await Promise.all(
		activitySummaries.map(async (activitySummary) => {
			let response = await fetch(`https://www.strava.com/api/v3/activities/${activitySummary.id}`, {
				headers: {
					Authorization: 'Bearer ' + get(token)?.access_token
				}
			});
			let activity: Activity = await response.json();
			return activity;
		}
	));
	return activities;
...
export const load = ( async ({ fetch, url }) => {
	...
	const activities = await getActivities(fetch, 1, 60, Math.floor(endOfWeek / 1000), Math.floor(startOfWeek/1000));
